...
def resolve_proxy_url(
    ...
    *platform_env_var* is a per-adapter, per-profile-configurable setting (each proxy URL can
    ...
    from gateway.platforms._shared import (
        get_scoped_secret as _get_scoped_proxy_var,
        profile_scoped,
    )
    ...
    if not value:
        # A multiplexed secondary profile with no scoped/configured proxy must
        # not inherit the launch profile's process env or macOS system proxy.
        if profile_scoped():
            return None
