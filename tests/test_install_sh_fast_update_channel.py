...
def _seed_python_tree(
    root: Path,
    importable: bool = True,
    missing_yaml: bool = False,
    missing_fastapi: bool = False,
) -> None:
    ...
    elif missing_fastapi:
        python.write_text('#!/bin/bash\ncase "$*" in *fastapi*) exit 1 ;; *) exit 0 ;; esac\n')
    else:
...
def test_python_marker_cannot_hide_missing_desktop_gateway(tmp_path: Path) -> None:
    """A lock fingerprint cannot vouch for a runtime that would die before
    the Desktop gateway binds because FastAPI is missing."""
    _seed_python_tree(tmp_path, missing_fastapi=True)
    res = _run_py_trio(
        'python_deps_mark_installed "$PWD"\n'
        'python_deps_up_to_date "$PWD" && echo UP_TO_DATE || echo NEEDS_INSTALL',
        tmp_path,
