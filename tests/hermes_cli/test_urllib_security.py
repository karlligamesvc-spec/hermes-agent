...
def _isolate_loopback_requests_from_machine_proxies(monkeypatch):
    """Keep wire tests on their in-process source and sink servers."""
    for name in (
        "HTTP_PROXY",
        "HTTPS_PROXY",
        "ALL_PROXY",
    ...
    # urllib on macOS can also discover proxies from System Configuration.
    # An explicit wildcard bypass prevents that path from catching localhost.
