...
def test_console_cancel_stops_forked_agent_request_before_reporting(console_client, monkeypatch, blocking_provider, stop):
    ...
    if stop == "timeout":
        # Agent/plugin initialization is deliberately outside this assertion. A 2s
        # deadline can expire before the request reaches the blocking provider under
        # full-suite load, testing startup contention instead of cancellation cleanup.
        monkeypatch.setattr(chat_ws, "_CONSOLE_COMMAND_TIMEOUT_SECONDS", 15.0)
