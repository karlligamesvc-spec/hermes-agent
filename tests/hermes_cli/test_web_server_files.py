...
def test_download_resolves_paths_in_the_originating_profile_session(
    ...
    for profile, sid, cwd in [
        ("default", "origin-session", str(session_cwd)),
        ("other", "other-session", str(gateway_cwd)),
    ]:
        db_home = (
            hermes_home if profile == "default" else hermes_home / "profiles" / profile
        )
    ...
    for route in ("/api/fs/download", "/api/fs/read-data-url"):
        for path in ("./report.txt", "../project/report.txt", str(artifact), artifact.as_uri()):
            ...
            data = (
                base64.b64decode(response.json()["dataUrl"].split(",", 1)[1])
                if route.endswith("read-data-url")
                else response.content
            )
            assert data == artifact.read_bytes()
        for profile, session_id in (
            ("other", "origin-session"),
            ("missing", "origin-session"),
            ("default", "missing-session"),
            ("default", ""),
        ):
            response = client.get(
                route,
