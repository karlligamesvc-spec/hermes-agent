...
def test_helper_stdout_is_returned_verbatim(tmp_path):
    ...
    # This case verifies byte-for-byte stdout, not the timeout path. Keep enough
    # headroom for a heavily parallel suite; timeout behavior has dedicated tests.
    value = _run_helper(str(helper), "CMDTEST_API_KEY", 10.0, 1024)
