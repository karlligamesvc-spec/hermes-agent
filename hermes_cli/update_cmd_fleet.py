...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    launchd_failed = any(name == "launchd" or name.startswith("ai.hermes.") for name in ordered)
    systemd_failed = any(name.startswith("hermes-") or name.startswith("systemd-") for name in ordered)
    print()
