...
def _codex_login_post(url: str, *, failure: Tuple[str, str], **kwargs: Any) -> "httpx.Response":
    ...
    try:
        with _codex_http_client(base_url=url, timeout=httpx.Timeout(15.0)) as client:
            return client.post(url, **kwargs)
    except Exception as exc:
        raise _codex_err(f"{failure[0]}: {exc}{_ssl_interop_hint(exc)}", failure[1]) from exc


def _codex_http_client(*, base_url: str = "", **kwargs: Any) -> "httpx.Client":
    ...
    per RFC 8305) works.
    """
    from agent.proxy_bypass import is_loopback_url
...
def refresh_codex_oauth_pure(
    ...
    with _codex_http_client(
        base_url=CODEX_OAUTH_TOKEN_URL,
...
def _probe_codex_quota_restored(
    ...
    try:
        ...
        usage_url = _codex_usage_probe_url(base_url)
        with _codex_http_client(base_url=usage_url, timeout=10.0) as client:
...
def _codex_poll_authorization_code(
    ...
    code_resp = None
    try:
        with _codex_http_client(base_url=issuer, timeout=httpx.Timeout(15.0)) as client:
