...
def run_matrix(surfaces):
    ...
    rows = []
    sync_http = build_keepalive_http_client(url)
    client = OpenAI(base_url=url, api_key="local-fixture", max_retries=0, http_client=sync_http)
    ...
    async def aux_async():
        async_http = build_keepalive_http_client(url, async_mode=True)
        async with AsyncOpenAI(
            base_url=url, api_key="local-fixture", max_retries=0, http_client=async_http,
        ) as async_client:
