...
const DESKTOP_LAUNCH = initializeDesktopLaunchEnvironment({
  additionalProtectedRoots: [
    productionHermesHomeFromEnvironment(process.env.HERMES_HOME, process.env[DIAGNOSTIC_TRIAL_ROOT_ENV]),
...
const USER_DATA_OVERRIDE = IS_DIAGNOSTIC_TRIAL ? DESKTOP_LAUNCH.userDataDir : process.env.HERMES_DESKTOP_USER_DATA_DIR
...
//
// Defaults:
//   Windows: %LOCALAPPDATA%\apexnodes
//   macOS / Linux: ~/.apexnodes
//
// Special case for Windows: if the user has a legacy ~/.apexnodes directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\apexnodes yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// HERMES_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// HERMES_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.apexnodes / %LOCALAPPDATA%\apexnodes.
...
// daemon's agent runner — inherits the SAME independent home instead of
// re-deriving its own ~/.hermes default. Idempotent when HERMES_HOME came from
// an explicit override; on a default install it pins clone/config/data under
// ~/.apexnodes.
...
function pathWithHermesManagedNode(...entries) {
  return [
    ...hermesManagedNodePathEntries(HERMES_HOME),
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `hermes:openExternal`, not here.
...
function spawnSecondaryWindow({
  ...
  watch,
  newSession
}: { sessionId?: string; profile?: null | string; watch?: boolean; newSession?: boolean } = {}) {
...
ipcMain.handle('hermes:bootstrap:continue-local', async () => continueLocalBootstrapIpc(continueFirstRunLocalBootstrap))
...
async function getUninstallSummary() {
  return runDesktopMaintenanceForPolicy(DESKTOP_LAUNCH_POLICY, 'uninstall-summary', getUninstallSummaryForProduction)
}
...
function healConfigYamlProductBlocks(reason) {
  try {
    ...
    // Managed relay discovery is a platform invariant; endpoints on other URLs
    // remain untouched because the helper requires the exact managed URL and
    // current credential. This pass is live, so a settings full-record save
    // cannot re-persist the collapsed one-model catalog for the rest of the run.
    if (managed.key && managed.baseUrl) {
      const relayHeal = syncManagedRelayConfigYaml(raw, managed.baseUrl, managed.key)

      if (relayHeal.changed) {
        raw = relayHeal.next

        const keyAnchors = relayHeal.key.anchors
          .filter(anchor => anchor.status !== 'in-sync')
          .map(anchor => anchor.path)

        const catalogAnchors = relayHeal.catalog.anchors
          .filter(anchor => anchor.status !== 'in-sync')
          .map(anchor => anchor.path)

        if (keyAnchors.length) {
          fixed.push(`managed-key(${keyAnchors.join(' ')})`)
        }

        if (catalogAnchors.length) {
          fixed.push(`managed-discovery(${catalogAnchors.join(' ')})`)
        }
      }
    }
