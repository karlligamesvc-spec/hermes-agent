...
`.trim()
  ...
  // `mutexPath` is already an expandRemotePath() shell word (for example
  // `"$HOME"'/.hermes-update-in-progress.mutex'`). Quoting it again would
  // pass the quote characters themselves to Python and create a literal
  // `'<absolute path>'` tree under the remote process cwd.
