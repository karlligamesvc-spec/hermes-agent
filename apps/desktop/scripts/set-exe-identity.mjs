#!/usr/bin/env node
// set-exe-identity.mjs — stamp the APEX icon + version metadata onto the
// built APEX.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked APEX.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-APEX.exe>
//
// Exits 0 on success, non-zero on failure when run as a CLI. As a hook,
// stampExeIdentity() resolves on success and rejects on failure. The rejection
// is intentionally fatal: an installer with stock Electron identity is not a
// valid APEX artifact.
...
import { windowsExeIdentity } from './windows-exe-identity.mjs'
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'file-version': identity.FileVersion,
