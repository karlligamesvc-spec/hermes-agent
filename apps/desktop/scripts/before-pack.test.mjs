...
test('preserveRollbackBackup moves a working build to .bak', () => {
  ...
  try {
    ...
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'APEX.exe'), 'MZ-old-build', 'utf8')
    fs.writeFileSync(path.join(appOutDir, 'resources.pak'), 'x', 'utf8')

    const preserved = preserveRollbackBackup(appOutDir, 'APEX.exe')
    ...
    assert.equal(
      fs.readFileSync(path.join(`${appOutDir}.bak`, 'APEX.exe'), 'utf8'),
      'MZ-old-build'
    )
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true })
...
test('preserveRollbackBackup replaces a stale .bak from an older update', () => {
  ...
  try {
    const appOutDir = path.join(tempRoot, 'win-unpacked')
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'APEX.exe'), 'current', 'utf8')
    fs.mkdirSync(`${appOutDir}.bak`, { recursive: true })
    fs.writeFileSync(path.join(`${appOutDir}.bak`, 'APEX.exe'), 'two-updates-ago', 'utf8')

    assert.equal(preserveRollbackBackup(appOutDir, 'APEX.exe'), true)
    assert.equal(fs.readFileSync(path.join(`${appOutDir}.bak`, 'APEX.exe'), 'utf8'), 'current')
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true })
  }
})
...
test('preserveRollbackBackup refuses a partial tree missing the product exe', () => {
  ...
  try {
    ...
    assert.equal(preserveRollbackBackup(appOutDir, 'APEX.exe'), false)
...
test('beforePack on win32 preserves the previous build instead of wiping it', async () => {
  ...
  try {
    const appOutDir = path.join(tempRoot, 'win-unpacked')
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'APEX.exe'), 'MZ-working', 'utf8')

    // No packager info in the context → current default 'APEX.exe' product name.
...
test('beforePack on linux keeps the plain wipe (no .bak)', async () => {
  ...
  try {
    const appOutDir = path.join(tempRoot, 'linux-unpacked')
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'APEX.exe'), 'x', 'utf8')
