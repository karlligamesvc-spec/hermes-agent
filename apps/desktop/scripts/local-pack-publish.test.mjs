...
describe('local desktop pack stays out of the publish path', () => {
  ...
  test('publish resolution succeeds with a GitHub token present', async () => {
    ...
    // APEX Desktop releases from our fork. Falling back to the upstream owner
    // would publish (or query) the wrong updater feed.
    assert.equal(configs[0].owner, 'karlligamesvc-spec')
