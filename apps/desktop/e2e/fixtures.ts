...
const PACKAGED_PRODUCT_NAME = DESKTOP_PACKAGE.build?.productName || DESKTOP_PACKAGE.productName || 'APEX'
const PACKAGED_EXECUTABLE_NAME = DESKTOP_PACKAGE.build?.executableName || PACKAGED_PRODUCT_NAME
...
function resolvePackagedBinaryPath(): string {
  ...
  return path.join(RELEASE_ROOT, 'linux-unpacked', PACKAGED_EXECUTABLE_NAME)
}
