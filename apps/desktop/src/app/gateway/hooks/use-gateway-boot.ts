...
import type { DesktopBootProgress, DesktopBootstrapEvent, HermesConnection } from '@/global'
...
// A real first install can legitimately outlive the normal backend-spawn
// budget. Stay attached to the SAME getConnection IPC only while Electron
// proves that an active/recently-completed bootstrap owns this attempt. A
// streaming install is bounded by inactivity, not total wall time: healthy
// Windows dependency resolution has exceeded an hour.
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    const cancelStalledBootstrap = () => {
      ...
      try {
        ...
        // Compatibility bridges can throw synchronously. The finite grace
        // still releases the renderer when cancellation is unavailable.
      }
