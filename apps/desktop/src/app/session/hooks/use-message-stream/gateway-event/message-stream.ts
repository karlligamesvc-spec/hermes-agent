...
export function handleMessageStreamEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'moa.reference') {
    // A selected model's answer, streamed ahead of the acting model's reply.
    // Keep upstream's reference-model vocabulary off the user surface.
    if (sessionId) {
      ...
      const header = advisoryBlockHeader(label, idx, cnt)
