...
interface ModelCatalogMenuProps {
  ...
  includeMoa?: boolean
  /** Optional product-level catalog policy. Shared surfaces omit this and see
   *  the runtime catalog verbatim; APEX's composer supplies its China-first
   *  allowlist without forking the catalog renderer. */
  providerFilter?: (providers: ModelOptionProvider[]) => ModelOptionProvider[]
...
export function ModelCatalogMenu({
  ...
  const providers = useMemo(
    () => (providerFilter ? providerFilter(rawProviders ?? []) : rawProviders),
    [providerFilter, rawProviders]
  )
