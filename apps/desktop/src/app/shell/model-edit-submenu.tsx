...
interface ModelEditSubmenuProps {
  ...
  provider: string
  /** Optional display/provider hint used to resolve the vendor effort scale. */
  providerName?: string
...
export function ModelOptionsContent({
  ...
  // hc-598: vendors expose different effort scales. Keep upstream's pure
  // controller boundary, but don't reintroduce seven misleading choices for
  // every model while adopting it.
  const effortOptions = supportedReasoningEfforts(model, providerName || provider)
  const inheritedEffort = resolveReasoningEffort(effort, defaultEffort)
  const effortValue = inheritedEffort ? nearestSupportedEffort(inheritedEffort, effortOptions) : ''
        ...
        <DropdownMenuItem className={dropdownMenuRow} onSelect={event => event.preventDefault()}>
          ...
          <Switch
            ...
            onCheckedChange={checked =>
              onSetOptions({
                effort: checked ? effortValue || nearestSupportedEffort(defaultEffort, effortOptions) : 'none'
              })
            }
            size="xs"
          />
