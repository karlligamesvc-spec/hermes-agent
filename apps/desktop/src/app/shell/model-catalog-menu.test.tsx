...
beforeEach(() => {
  ...
  // Reset both call history and any queued one-shot implementation. A late
  // catalog refresh from the preceding test can otherwise leave a consumed
  // `mockResolvedValueOnce` ahead of this test's fixture and repaint the menu
  // with the default provider list.
  getGlobalModelOptions.mockReset().mockResolvedValue({
...
describe('in-flight local downloads', () => {
  ...
  it('shows the download inside the Local provider group when it exists', async () => {
    ...
    const download = await screen.findByText('Qwen3.8 Flash Next (UD-Q4_K_XL)')
    const localHeading = screen.getByText('Local')
    const localGroup = localHeading.closest('[role="group"]')

    expect(localGroup?.textContent).toContain('Qwen3.6')
    expect(localGroup?.contains(download)).toBe(true)
