...
export function recentConversations(
  ...
  return [...sessions]
    ...
    .slice(0, limit)
    .map(session => ({
      // Match sidebar/search/command-center restore semantics. The lineage
      // root scopes pins and composer state; session.resume opens the tip id.
      id: session.id,
...
/**
 * Read evidence through the same task/artifact seams as their canonical pages.
 * Partial failures stay partial: callers can show an honest limitation while
 * preserving the working conversations, Tasks, and Artifacts routes.
 */
