...
export interface DesktopOnboardingState {
  ...
  /** Compatibility state for upstream's optional first-run free-tier card.
   *  APEX's managed route does not advertise plans or prices, but the shared
   *  onboarding shell still needs the field to remain type-compatible. */
...
/** A profile may emit its provider warning while the user is only browsing it.
 * Opening onboarding here would punish inspection, so defer the warning until
 * submit. The latest warning wins; a healthy event clears the pending gate. */
...
async function applyFreeTierIntro(ctx: OnboardingContext, runtime: RuntimeReadinessResult) {
  // APEX is currently a free, managed product and does not expose upstream's
  // plan/price onboarding. Keep the compatibility state wired but default the
  // upstream surface off so a runtime capability refresh cannot reveal it.
  const APEX_FREE_TIER_INTRO_ENABLED = false

  if (!APEX_FREE_TIER_INTRO_ENABLED) {
    setFreeTierRoute(null)
...
async function pollSession(
  ...
  start: DeviceStart | LoopbackStart,
  ctx: OnboardingContext,
  generation: number
) {
  try {
