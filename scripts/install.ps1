    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                ...
                # Persist with the managed tree first so a stale system Node
                # cannot win in fresh shells or later bootstrap stages.
                Set-ManagedNodeFirstOnUserPath "$HermesHome\node"
...
function Install-SystemPackages {
    ...
    # -- hc-632: COS mirror first, package managers only as fallback ----------
    # 2026-07-31, a real first install on a mainland-China Windows box: every
    # MANDATORY stage finished fast (uv 3.7s / python 12s / git 1.4s / node
    # 0.1s), because each already had a China route -- uv/git/runtime from our
    # COS, PyPI from TUNA, CPython/Node/npm/Electron/Playwright from npmmirror.
    # Then the install sat 18+ minutes here, on `winget install --source
    # winget`. That source is GitHub/Microsoft-CDN backed and was the ONLY
    # download left in the entire chain without a China mirror. The user saw a
    # progress bar stuck on "prerequisites" with no hint what it waited on.
    #
    # So we mirrored the two artifacts and install from them, same as uv and
    # git: the CN path now gets a COMPLETE install in seconds rather than a
    # faster degraded one. Region-independent by design (hc-474) -- every
    # install with HERMES_RUNTIME_COS_BASE set takes the COS route, which is
    # also the only reason this path is exercised enough to stay working.
    #
    # Publisher: scripts/mirror-optional-packages.py (refuses to upload a zip
    # that fails CRC or is missing the expected .exe).
    if (Get-Command Install-OptionalPkgFromCos -ErrorAction SilentlyContinue) {
        $installedAny = $false
        if ($needRipgrep -and (Install-OptionalPkgFromCos -Package 'ripgrep')) { $installedAny = $true }
        if ($needFfmpeg  -and (Install-OptionalPkgFromCos -Package 'ffmpeg'))  { $installedAny = $true }
        if ($installedAny) {
            Register-ApexToolsPath
            # Re-check through PATH instead of trusting the return values.
            # "the copy succeeded" and "the runtime can find it" are different
            # claims, and shutil.which() is the one that decides at run time
            # (tools/tts_tool.py, tools/file_operations.py).
...
function Install-Dependencies {
    ...
    try {
    # APEX CN mode can preserve the same uv.lock hashes while fetching from a
    # reachable mirror. It must run before the official-host locked sync,
    # which is valid but can take tens of minutes on mainland networks.
