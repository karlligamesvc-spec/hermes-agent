...
def create_ssrf_safe_async_client(**kwargs: Any) -> Any:
    ...
    certificate verification. Explicit environment proxies delegate resolution to the proxy;
    implicit platform proxy discovery stays disabled so macOS system settings cannot silently
    bypass the guarded direct transport."""
    import httpx
    kwargs.setdefault("trust_env", _proxy_is_configured())
