...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            # Cache raw provider results, then apply the operator policy per
            # call so a configuration change never leaves blocked hosts in a
            # memoized response. Filtering before slicing avoids returning a
            # blocked candidate simply because it fell inside the limit.
