...
def _filter_search_results_by_policy(response_data: dict) -> dict:
    """Drop blocklisted search hits before they can be handed to extract."""
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            response_data = _filter_search_results_by_policy(
                _memoized_search(provider, query, limit)
            )
...
async def web_extract_tool(urls: List[Any], format: str = None, char_limit: Optional[int] = None) -> str:
    ...
    try:
        ...
        safe_urls, safe_indices, ssrf_blocked, policy_blocked = [], [], {}, {}
        ...
        if invalid_urls or ssrf_blocked or policy_blocked:
            fixed = {**ssrf_blocked, **policy_blocked, **invalid_urls}
