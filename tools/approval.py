...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    # Compare the spelling against the platform's advertised temp directory
    # before resolving it. On macOS /tmp is a symlink to /private/tmp; comparing
    # the raw operand with the resolved directory incorrectly rejected our own
    # cleanup while this exact-shape check still rejects traversal/globs.
    # `/tmp` -> `/private/tmp` is the standard macOS system alias used by
    # tempfile and shell commands. Do not extend this exemption to arbitrary
    # symlinked temp roots: their alias spelling remains approval-gated.
    accepted_dir = (
        raw_temp_dir
        if raw_temp_dir == "/tmp" and temp_dir == "/private/tmp"
        else temp_dir
    )
    if operand != os.path.join(accepted_dir, basename):
        return False
