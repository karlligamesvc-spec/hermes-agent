    ...
    async def _cleanup_process(self) -> None:
        async with self._cleanup_lock:
            ...
            if proc is None or proc.returncode is not None:
                return
            # The LSP ``exit`` notification normally makes the server stop by itself. Give
            # asyncio one short window to observe that exit before sending SIGTERM. Without
            # this grace, a just-exited child can be reaped/reparented between the returncode
            # check and ``terminate()``, creating a PID-reuse race in long-running hosts.
            try:
                try:
                    await asyncio.wait_for(proc.wait(), timeout=min(SHUTDOWN_GRACE, 0.1))
                    return
