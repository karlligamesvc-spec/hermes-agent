...
def _action_lines(data: Dict, detail: Dict, verbose: bool) -> List[str]:
    ...
    if data.get("staged"):
        ...
        if data.get("proposal_staged") and data.get("message"):
            return [data["message"]]

        # hc-376: approval-gated writes are queued, not applied. A real gate result carries a
        # durable pending_id; generic staged/dry-run results keep upstream's silent semantics.
        # Report the gated state honestly instead of describing the requested mutation as landed.
        if data.get("pending_id"):
            if detail.get("tool") == "skill_manage":
                name = detail.get("name", "") or data.get("gist", "")
                if name:
                    return [
                        f"📋 Skill '{name}' staged for approval "
                        "(review with /skills pending)"
                    ]
                return ["📋 Skill change staged for approval (review with /skills pending)"]
            return ["📋 Memory change staged for approval (review with /memory pending)"]
        return []
