...
def build_anthropic_client(api_key, base_url: str = None, timeout: float = None, *, drop_context_1m_beta: bool = False):
    ...
    normalize_proxy_env_vars()
    normalized_base_url, kwargs = _base_client_kwargs(base_url, timeout)
    # Match the OpenAI transport policy: only explicit proxy environment variables may route
    # provider traffic.  A stock httpx client also imports the macOS system proxy, but httpx does
    # not honor that proxy's ExceptionsList; localhost/custom LAN endpoints can therefore be sent
    # to the proxy and fail with a synthetic 502 before the provider ever sees the request.
    from agent.process_bootstrap import build_keepalive_http_client
    http_client = build_keepalive_http_client(normalized_base_url)
